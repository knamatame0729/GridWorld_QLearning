...
class DoubleDQN(nn.Module):
    """
    Double Deep Q-Network (DQN) with 2 hidden layers and ReLU activations
...
class Agent:
    ...
    def __init__(self):
        ...
        # Networks
        self.policy_net = DoubleDQN(input_dim, output_dim).to(DEVICE)
        self.target_net = DoubleDQN(input_dim, output_dim).to(DEVICE)
    ...
    def chooseAction(self, epsilon=None):
        ...
        # If epsilon value is defined, use it otherwise use EPSILON_START for epsilon
...
def main():
    ...
    run_name = f"DDQN_lr_{LEARNING_RATE}_gamma_{GAMMA}_epsilon_decay_{EPSILON_DECAY}"
